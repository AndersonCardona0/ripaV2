...
const STORAGE_KEY = 'artisanal_grupos_mesas';

// ─── Helpers de localStorage ─────────────────────────────────────────────────
function guardarGrupos(mesasData, GROUP_SIZE) {
    const grupos = [];
    for (let i = 0; i < mesasData.length; i += GROUP_SIZE) {
        grupos.push(mesasData.slice(i, i + GROUP_SIZE).map(m => m.id));
    }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    let mesasData  = [];
    let grupoMode  = !!leerGrupos();   // restaurar desde localStorage al cargar
    const GROUP_SIZE = 6;

    // Sincronizar botón con el estado restaurado desde localStorage
    function sincronizarBoton() {
        const btn = document.getElementById('btn-toggle-grupo');
        if (!btn) return;
        const label = btn.querySelector('.btn-grupo-label');
        if (label) label.textContent = grupoMode ? 'Vista Normal' : 'Agrupar';
        if (grupoMode) {
            btn.classList.add('text-[#BC5F40]', 'border-orange-200', 'bg-orange-50');
            btn.classList.remove('text-gray-400', 'border-gray-200', 'bg-white');
        } else {
            btn.classList.remove('text-[#BC5F40]', 'border-orange-200', 'bg-orange-50');
            btn.classList.add('text-gray-400', 'border-gray-200', 'bg-white');
        }
    }
    sincronizarBoton();

    // ── Clases de borde para tarjetas fusionadas ─────────────────────────────
    function borderClases(pos) {
        switch (pos) {
            case 'first': return 'rounded-l-2xl rounded-r-none';
            case 'mid':   return 'rounded-none border-l-0';
            case 'last':  return 'rounded-r-2xl rounded-l-none border-l-0';
            default:      return 'rounded-3xl';
        }
    }

    // ── HTML de cada tarjeta ──────────────────────────────────────────────────
    // En modo agrupado, w-1/6 garantiza ancho uniforme sin importar cuántas
    // tarjetas queden en la fila (evita el estiramiento de mesas huérfanas).
    function crearTarjetaHtml(mesa, pos) {
        const esDisponible = mesa.estado === 'disponible' || mesa.estado === 'libre';
        const bc   = borderClases(pos);
        const size = grupoMode ? 'flex-none w-1/6' : '';

        if (esDisponible) {
            return `
                <div class="bg-white ${bc} ${size} p-6 border border-stone-100 shadow-sm flex flex-col cursor-pointer hover:shadow-md transition-shadow"
                    ...
                    <div class="flex justify-between items-center">
                        ...
                        <button class="w-8 h-8 bg-[#BC5F40] text-white rounded-full flex items-center justify-center text-lg font-bold hover:bg-[#a04e35] transition-colors shadow-sm">+</button>
                    </div>
                </div>`;
        } else {
            return `
    ...
    // ── Renderiza una fila de grupo con border-radius inteligente ─────────────
    function renderGrupoFila(grupo) {
        const fila = document.createElement('div');
        fila.className = 'flex';
        grupo.forEach((mesa, idx) => {
            let pos;
            if (grupo.length === 1)           pos = 'solo';
            else if (idx === 0)               pos = 'first';
            else if (idx === grupo.length - 1) pos = 'last';
            else                              pos = 'mid';
            fila.insertAdjacentHTML('beforeend', crearTarjetaHtml(mesa, pos));
        });
    ...
    function renderMesas() {
        ...
        mesasData.forEach(m => {
            if (m.estado === 'disponible' || m.estado === 'libre') disponibles++;
            else ocupadas++;
        });

        if (grupoMode) {
            contenedor.className = 'flex flex-col gap-3';

            const grupos = leerGrupos();
            if (grupos && grupos.length > 0) {
                // Reconstruir grupos usando IDs guardados → respeta el orden tras navegar
                const mesaMap = Object.fromEntries(mesasData.map(m => [m.id, m]));
                grupos.forEach(grupoIds => {
                    const grupo = grupoIds.map(id => mesaMap[id]).filter(Boolean);
                    if (grupo.length > 0) renderGrupoFila(grupo);
                });
                // Añadir mesas que no estaban en los grupos guardados (altas nuevas)
                const savedIds = new Set(grupos.flat());
                const nuevas   = mesasData.filter(m => !savedIds.has(m.id));
                for (let i = 0; i < nuevas.length; i += GROUP_SIZE) {
                    renderGrupoFila(nuevas.slice(i, i + GROUP_SIZE));
                }
            } else {
                // Sin datos guardados: agrupación secuencial
                for (let i = 0; i < mesasData.length; i += GROUP_SIZE) {
                    renderGrupoFila(mesasData.slice(i, i + GROUP_SIZE));
                }
            }
        } else {
            contenedor.className = 'grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6';
            mesasData.forEach(mesa => {
                contenedor.insertAdjacentHTML('beforeend', crearTarjetaHtml(mesa, 'solo'));
            });
        }
        ...
        fetch('/controllers/api_mesas.php')
            ...
            .then(res => {
                ...
                res.data.forEach(mesa => {
                    ...
                    if (!agrupadas[num]) {
                        agrupadas[num] = { ...mesa, total_balance: parseFloat(mesa.total_balance) || 0 };
                    } else {
                        agrupadas[num].total_balance += parseFloat(mesa.total_balance) || 0;
                    }
                });
                mesasData = Object.values(agrupadas);

                // Mantener localStorage sincronizado si el agrupado está activo
                if (grupoMode) guardarGrupos(mesasData, GROUP_SIZE);
    ...
    // ── Toggle agrupado ───────────────────────────────────────────────────────
    window.toggleGrupoMode = function () {
        grupoMode = !grupoMode;

        if (grupoMode) {
            guardarGrupos(mesasData, GROUP_SIZE);
        } else {
...
window.abrirConfiguracionSalon = function() {
    ...
    const totalMesasActuales = gridMesas
        ? gridMesas.querySelectorAll('[onclick^="irAMesa"]').length
        : 0;
...
// Soporte teclado físico cuando el modal PIN está visible
